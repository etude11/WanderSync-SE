...
const SEV_META: Record<DisruptionSeverity, { label: string; color: string; bg: string; border: string }> = {
...
export default function DisruptionList({ disruptions }: DisruptionListProps) {
  ...
  return (
    <div className="space-y-3">
      {disruptions.map((d, i) => {
        const sev = SEV_META[d.severity] ?? SEV_META.MEDIUM;
        return (
          <div
            key={d.id}
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-charcoal/35 mt-1.5 font-mono">
                    Detected {new Date(d.detectedAt).toLocaleString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>

              {d.resolved && (
                <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0" style={{ background: 'rgba(34,56,67,0.07)', color: '#223843' }}>
                  <CheckIcon /> Resolved
                </span>
              )}
            </div>
