...
WanderSync is decomposed into seven top-level subsystems connected through explicit interfaces:
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Authentication \& User Management Service} — registration, login, JWT issuance,
        profile CRUD.
  \item \textbf{Booking Aggregation Service} — \texttt{IBookingStrategy} per provider; normalises
  ...
  \item \textbf{Notification Service} — dedicated Redis Streams consumer; delivers in-app and
        email alerts to affected travellers within the 10-second NFR2 window.
\end{enumerate}

The Disruption Detection Service, Itinerary Management Service, and Notification Service
communicate exclusively through the message broker, realising the event-driven decoupling
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Message Broker (Redis Streams)} — Disruption Detection publishes
        \texttt{DisruptionEvent} messages; Itinerary Management and Notification Service
        subscribe as independent consumer groups.
  ...
  \item \textbf{Redis (Data Layer)} — serves two roles: (a) the Redis Streams channel used by
        the message broker above; (b) the short-lived response cache used by the Booking
        Aggregation Service (60-second TTL, Tactic 1) and the itinerary cache in the Itinerary
        Management Service. Redis is a critical state store and is classified as part of the
        data layer alongside the relational databases.
