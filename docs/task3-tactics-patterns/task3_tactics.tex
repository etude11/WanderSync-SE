...
gateway, which appends a decoded identity header after token validation.

Role-based authorisation (traveller, admin) is also enforced at the gateway, preventing
downstream services from duplicating auth logic and ensuring consistent policy enforcement. A
single middleware update at the gateway propagates to all services simultaneously.

\textbf{Trade-off:} The gateway becomes a single enforcement point, which means a gateway
misconfiguration could expose all services. This risk is mitigated by keeping the auth middleware
